                ...
                ZStack(alignment: .topLeading) {
                    if let image = snapshotImage {
                        Image(uiImage: image)
                            .resizable()
                            .blur(radius: 30)
                            .frame(width: geometry.size.width - 40, height: (geometry.size.width - 40) * 210 / 360)
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            HStack {
                                Text(lycée.name)
                                    .font(.system(size: 16, weight: .medium, design: .rounded).bold())
                            ...
                            if let image = snapshotImage {
                                Image(uiImage: image)
                                    .resizable()
                                    .frame(width: geometry.size.width - 54, height: (geometry.size.width - 66) * 138 / 350)
                                    .clipShape(RoundedRectangle(cornerSize: CGSize(width: 20, height: 30)))
